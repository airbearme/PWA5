...
/**
 * ⚡ Bolt: Optimized utility functions moved outside the component scope
 * to prevent re-creation on every render.
 */
const calculateDistance = (spot1: Spot, spot2: Spot): number => {
  const R = 6371; // Earth's radius in km
  // Explicitly cast to Number to handle DECIMAL types that might be returned as strings
...
export default function BookRidePage() {
   ...
   * Spots are fetched once on mount. Synchronization from URL is handled in a separate effect.
   */
  useEffect(() => {
  ...
  // Handle pickup spot synchronization from URL search parameters
  useEffect(() => {
  ...
  /**
   * ⚡ Bolt: Memoized derived values to prevent redundant calculations on every render.
   */
