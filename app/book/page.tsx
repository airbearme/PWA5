...
/**
 * Utility: Calculate distance between two spots using Haversine formula
 * Moved outside component to prevent re-creation on every render
 */
const calculateDistance = (spot1: Spot, spot2: Spot): number => {
  const R = 6371; // Earth's radius in km
  // Ensure we're working with numbers (compatibility with DECIMAL types as strings)
...
/**
 * Utility: Estimate fare based on distance
 * Moved outside component to prevent re-creation on every render
 */
...
export default function BookRidePage() {
  ...
  // Optimization: Fetch spots once on mount only to avoid redundant network requests
  useEffect(() => {
  ...
  // Sync pickup spot from URL search parameters separately
  useEffect(() => {
  ...
  /**
   * Performance Optimization: Memoize distance and fare to prevent
   * redundant calculations during UI updates (e.g., input typing, animations)
   */
