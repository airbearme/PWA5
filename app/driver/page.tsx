...
export default function DriverDashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Parallelize ride fetching & conditionally fetch spots to optimize network & reduce latency
      const [pendingRidesResult, activeRideResult, spotsResult] = await Promise.all([
        supabase
          .from("rides")
          .select("*")
          .eq("status", "pending")
      ...
      if (spotsResult.data && !spotsFetchedRef.current) {
  ...
  useEffect(() => {
    ...
    // Refresh every 5 seconds using the memoized loadData function
  ...
  const handleAcceptRide = async (rideId: string) => {
    ...
    try {
      ...
      // ⚡ Bolt: Use memoized loadData() for instantaneous state update instead of a full page reload
      await loadData();
    } catch (error: any) {
      toast({
        title: "Error",
        ...
        <div className="text-center">
          <div className="flex justify-center mb-6">
            <div className="w-32 h-32 rounded-full border-4 border-emerald-400/50 dark:border-emerald-500/50 bg-gradient-to-br from-emerald-500/20 to-lime-500/20 backdrop-blur-sm shadow-2xl hover-lift animate-float overflow-hidden">
              {/* ⚡ Bolt: Use Next.js Image component with priority for critical LCP mascot */}
              <Image
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <div className="w-24 h-24 rounded-full border-4 border-emerald-400/50 dark:border-emerald-500/50 bg-gradient-to-br from-emerald-500/20 to-lime-500/20 backdrop-blur-sm shadow-2xl hover-lift animate-float overflow-hidden">
              {/* ⚡ Bolt: Use Next.js Image component for optimized mascot delivery */}
              <Image
                src="/airbear-mascot.png"
                alt="AirBear Mascot"
                width={96}
