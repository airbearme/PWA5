...
export default function DriverDashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // ⚡ Bolt: Parallelize data fetching to reduce dashboard latency.
      // Static lookup data (spots) is only fetched once using a useRef flag.
      const fetchPromises = [
        supabase
          .from("rides")
          .select("*")
        ...
        supabase
          ...
          .maybeSingle() // ⚡ Bolt: Use maybeSingle to avoid unnecessary exceptions if no active ride
      ] as const;

      const results = await Promise.all([
        ...fetchPromises,
        ...(!spotsLoaded.current ? [supabase.from("spots").select("id, name")] : [])
      ]);
  ...
  const handleAcceptRide = async (rideId: string) => {
    ...
    try {
      ...
      // ⚡ Bolt: Targeted state refresh instead of full window reload for better UX.
      await loadData();
    } catch (error: any) {
      toast({
        title: "Error",
