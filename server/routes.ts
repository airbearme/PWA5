...
if (!stripeSecretKey) {
	throw new Error(
		"STRIPE_SECRET_KEY is required for live Stripe, Apple Pay, and Google Pay.",
	);
}
...
export async function registerRoutes(app: Express): Promise<Server> {
	...
	const ensureUserProfile = async (payload: z.infer<typeof profileSchema>) => {
		...
		const existingUser = payload.id
			...
			: payload.email
				? await storage.getUserByEmail(payload.email)
				: null;
	...
	app.post("/api/auth/register", async (req, res) => {
		try {
			...
			const userData = profileSchema
				...
				.omit({ role: true }) // Sentinel: Prevent role escalation during registration
				.parse(req.body);
	...
	app.post("/api/auth/login", async (req, res) => {
		try {
			...
			const profile = await ensureUserProfile({
				...
				fullName:
					(data.user.user_metadata?.fullName as string | undefined) || null,
				role:
	...
	app.post("/api/auth/sync-profile", async (req, res) => {
		try {
			...
			const payload = profileSchema.omit({ role: true }).parse(req.body); // Sentinel: Prevent role escalation during sync
	...
	app.post("/api/payments/confirm-cash", async (req, res) => {
		try {
			...
			res
				.status(400)
				.json({
					message: "Invalid QR code or confirmation error: " + error.message,
				});
		}
	});
	...
	app.post("/api/ceo-tshirt/purchase", async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({
					message: "Error creating CEO T-shirt payment: " + error.message,
				});
		}
	});
			...
			switch (event.type) {
				case "payment_intent.succeeded": {
					...
					if (metadata?.orderId || metadata?.rideId) {
						...
						if (metadata.rideId) {
							await storage.updateRide(metadata.rideId, {
								status: "completed",
							});
						}
					}
	...
	app.get("/api/analytics/overview", async (req, res) => {
		try {
			...
			const activeAirbears = airbears.filter(
				(a) => a.isAvailable && !a.isCharging,
			);
			const chargingAirbears = airbears.filter((a) => a.isCharging);
			const maintenanceAirbears = airbears.filter(
				(a) => a.maintenanceStatus !== "good",
			);
			...
			const analytics = {
				...
				averageBatteryLevel:
					airbears.length > 0
						? Math.round(
								airbears.reduce((sum, a) => sum + a.batteryLevel, 0) /
									airbears.length,
							)
						: 0,
			};
	...
	app.post("/api/push-subscriptions", async (req, res) => {
		try {
			...
			if (!subscription || !preferences) {
				return res
					.status(400)
					.json({ message: "Subscription and preferences required" });
			}
	...
	app.patch("/api/push-subscriptions", async (req, res) => {
		try {
			...
			if (!endpoint || !preferences) {
				return res
					.status(400)
					.json({ message: "Endpoint and preferences required" });
			}
