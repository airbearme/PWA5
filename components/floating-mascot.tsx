...
export default function FloatingMascot() {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      // Update motion values without triggering React re-renders
  ...
  // Transform mouse position to a smaller range for the follow effect,
  // relative to the center of the screen for a parallax effect.
  const followX = useTransform(mouseX, (x) => (x - (typeof window !== 'undefined' ? window.innerWidth / 2 : 0)) * 0.05);
  const followY = useTransform(mouseY, (y) => (y - (typeof window !== 'undefined' ? window.innerHeight / 2 : 0)) * 0.05);

  const springX = useSpring(followX, springConfig);
  const springY = useSpring(followY, springConfig);

  return (
    <motion.div
      className="fixed bottom-6 right-6 z-50"
      // This style object uses motion values. Framer Motion will update the transform
      // directly on the DOM element, bypassing React's render cycle for a smooth animation.
      style={{
        translateX: springX,
        translateY: springY,
      }}
