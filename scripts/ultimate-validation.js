...
async function main() {
	...
	if (checkFileExists("tests/jest/integration.test.ts")) {
		runTest(
			"Integration Tests",
			"pnpm run test -- tests/jest/integration.test.ts",
			false,
		);
	} else {
	...
	if (checkFileExists("tests/jest/api.test.ts")) {
		runTest("API Tests", "pnpm run test -- tests/jest/api.test.ts", false);
	} else {
