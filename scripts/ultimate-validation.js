...
async function main() {
	...
	if (checkFileExists("__tests__/integration.test.ts")) {
		runTest(
			"Integration Tests",
			"pnpm run test -- __tests__/integration.test.ts --passWithNoTests",
			false,
		);
	} else {
	...
	if (checkFileExists("__tests__/api.test.ts")) {
		runTest("API Tests", "pnpm run test -- __tests__/api.test.ts --passWithNoTests", false);
	} else {
